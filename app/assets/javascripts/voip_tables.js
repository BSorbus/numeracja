...
$(document).ready(function() {
  ...
  function voipTableFilterColumnParams ( i ) {
    $('#voip_tables-datatable').DataTable().column( i ).search(
      $('#col'+i+'filter').val(), false, true, true
    );
    $('#voip_tables-datatable').DataTable().draw();
  };

  $.fn.dataTable.ext.buttons.voip_tables_button_filter_show_hide = {
    //text: 'Filtr',
...
//    dom: "<'row'<'col-sm-6'l><'col-sm-6'Bf>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-5'i><'col-sm-7'p>>",
    ...
    searchDelay: 500,
    ajax: { 
      url: $('#voip_tables-datatable').data('source'),
      type: "GET",
      data: function(d){
        //d.eager_filter_for_current_user = $('#voip_tables-datatable').DataTable().button(3).active();
      },
    ...
    order: [[ 1, "asc" ]],
    language: {
      //url: $('#voip_tables-datatable').data('locale') "<%= Rails.application.routes.url_helpers.datatables_lang_path(locale: I18n.locale, format: :json) %>"
      url: $('#voip_tables-datatable').data('locale_url')
    }
  });
 

  $('input.column_filter').on( 'keyup click', function () {
    voipTableFilterColumnParams( $(this).attr('data-column') );
  });
