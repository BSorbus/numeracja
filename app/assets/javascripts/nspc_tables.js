...
$(document).ready(function() {
  ...
  function nspcTableFilterColumnParams ( i ) {
    $('#nspc_tables-datatable').DataTable().column( i ).search(
      $('#col'+i+'filter').val(), false, true, true
    );
    $('#nspc_tables-datatable').DataTable().draw();
  };

  $.fn.dataTable.ext.buttons.nspc_tables_button_filter_show_hide = {
    //text: 'Filtr',
    text: '<span class="fa fa-eye-slash"></span>',
    titleAttr: 'Filtr',
    action: function ( e, dt, node, config ) {
      if ($('#nspc_tables-datatable').DataTable().tables().footer().to$().css('display') === 'none') {
        $('#nspc_tables-datatable').DataTable().tables().footer().to$().css('display', 'table-row-group');
  ...
  var oNspcTable = $('#nspc_tables-datatable').DataTable({
    dom: 'lBfrtip',
    buttons: [
      ...
      'nspc_tables_button_filter_show_hide'
    ],
    responsive: true,
    processing: true,
    serverSide: true,
  ...
  $('input.column_filter').on( 'keyup click', function () {
    nspcTableFilterColumnParams( $(this).attr('data-column') );
  });
