...
class NspcTable < ApplicationRecord

  def self.load_data_from_csv_file
    @buffer = []
    columns = [ :zone_ab, :number, :provider_id, :provider_name, :modification ]

    CSV.foreach("#{Rails.application.secrets.csv_files_path}/NSPC.csv", { 
